...
namespace :tracks do
    ...
    task import_all_data: :environment do
      ...
      csv_files.each do |file_path|
        import_trk_csv(file_path)
      end

      puts "All CSV files imported!"
    end

    # ex) rails tracks:import_dir_data[201904]
    desc "Import all CSV files under the specified directory in lib/data"
    task :import_dir_data, [:dirname] => :environment do |_t, args|
      base_path = Rails.root.join("lib", "data")
    ...
    # ex) rails tracks:import_file_data[201904/trk20190422_00_12.csv]
